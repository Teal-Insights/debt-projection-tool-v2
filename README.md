...
- `defaults.fcuShare`: sourced from the adjudicated DSA FX-share dataset where available (TEA-880). `src/data/fx-share.json` is generated by `npm run build:fx-share` from `data/fx-share/proposed-corrected-dataset.csv`, a verbatim copy of the TEA-878 adjudication package (167 countries; sha256-locked in the build script). Countries without a value keep 0, and the UI labels that 0 as an unsourced fallback under the Foreign currency debt share slider. Provenance (definition basis, debt perimeter, report URL/page/table) rides along per country.
