...
/**
 * Methodology page — modal-style overlay covering the chart UI. Reads in <3
 * minutes and tells a stakeholder everything they need to evaluate the tool:
 * purpose, FT origin, the math, data sources, assumptions (honest about the
 * illustrative fan bands), and credits.
 *
 * Accessibility:
 *   - role="dialog" + aria-modal="true" + aria-labelledby on the heading
 *   - Escape key closes
 *   - Initial focus moves to the close button so screen readers land there
 *   - Body scroll is locked while open (handled in App.tsx via a class)
 */
export function MethodologyPage({ onClose }: Props) {
  ...
  return (
    <div
      ...
      onClick={e => {
        // Click on backdrop (not inside the panel) closes
      ...
      <article className="methodology-page">
        <header className="methodology-page__header">
          ...
          <button
            ref={closeRef}
            type="button"
            className="methodology-page__close"
            onClick={onClose}
            aria-label="Close methodology page"
          >
            ✕
          </button>
        </header>
